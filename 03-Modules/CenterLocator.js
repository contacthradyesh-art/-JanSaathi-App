...
export default function CenterLocator({ language, onBack }) {
  ...
  useEffect(() => {
    ...
    async function loadLocation() {
      try {
        ...
        if (!servicesEnabled) {
          if (active) { setLocationState('fallback'); setErrorMessage(hi ? 'लोकेशन बंद है।' : 'Location services are off.'); }
          return;
        }
  ...
  const sortedCenters = useMemo(() => {
    if (!userLocation) return centers.map(center => ({ ...center, calculatedDistance: center.distance || '—' }));
    return centers
      .map(center => ({
        ...center,
        calculatedDistance: (typeof center.latitude === 'number' && typeof center.longitude === 'number')
          ? formatDistance(haversineKm(userLocation.latitude, userLocation.longitude, center.latitude, center.longitude))
          : '—',
        numericDistance: (typeof center.latitude === 'number' && typeof center.longitude === 'number')
    ...
    <View style={styles.screen}>
      <View style={styles.header}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerText}><Text style={styles.title}>{hi ? 'नज़दीकी सेंटर' : 'Nearby Centers'}</Text><Text style={styles.subtitle}>{userLocation ? (hi ? 'आपकी लोकेशन के अनुसार दूरी' : 'Distances from your location') : (hi ? 'सर्विस सेंटर की सूची' : 'Service center list')}</Text></View>
        </View>
      </View>
      {locationState === 'fallback' && <View style={styles.fallbackBanner}><Text style={styles.fallbackIcon}>ℹ️</Text><Text style={styles.fallbackText}>{hi ? 'Aapke paas ke centers dikhane ke liye location chahiye। अनुमति नहीं मिली, इसलिए static list दिखाई जा रही है।' : 'Location is needed to show nearby centers. Permission was not available, so the static list is shown.'}</Text></View>}
      <ScrollView contentContainerStyle={styles.content}>
        {sortedCenters.map((center, index) => (
          <View key={`${center.name}-${index}`} style={styles.card}>
            <View style={styles.nameRow}><View style={styles.iconBox}><Text style={styles.icon}>🏢</Text></View><Text style={styles.name} numberOfLines={2}>{center.name}</Text></View>
        ...
        <View style={styles.note}><Text style={styles.noteIcon}>ℹ️</Text><Text style={styles.noteText}>{userLocation ? (hi ? 'दूरी आपकी वर्तमान लोकेशन से लगभग है।' : 'Distances are approximate from your current location.') : (hi ? 'नोट: लोकेशन उपलब्ध न होने पर static दूरी दिखाई जाती है।' : 'Note: Static distances are shown when location is unavailable.')}</Text></View>
      </ScrollView>
    </View>
  );
}

function SafeAreaViewLike({ children }) {
